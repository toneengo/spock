...
void GrowableDescriptorAllocator::init(std::initializer_list<GrowableDescriptorAllocator::PoolSizeRatio> _ratios, uint32_t maxSets) {
    ratios      = _ratios;
...
void GrowableDescriptorAllocator::clear_pools() {
    for (auto p : pools) {
        vkResetDescriptorPool(ctx.device, p, 0);
    }
    currentPool = 0;
...
void GrowableDescriptorAllocator::destroy_pools() {
    for (auto p : pools) {
        vkDestroyDescriptorPool(ctx.device, p, nullptr);
    }
    currentPool = 0;
...
void GrowableDescriptorAllocator::set_flags(VkDescriptorPoolCreateFlags _flags) {
    flags = _flags;
}
