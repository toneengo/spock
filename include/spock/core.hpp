...
namespace spock {
    ...
    VkImageView           create_image_view(const char* dbgName, const spock::Image& image, VkImageViewType viewType, VkExtent3D extent, uint32_t baseMipLevel = 0, uint32_t mipLevels = 1, uint32_t baseArrayLayer = 0);
    inline VkImageView    create_image_view(const char* dbgName, const spock::Image& image, VkImageViewType viewType, VkExtent2D extent, uint32_t baseMipLevel = 0, uint32_t mipLevels = 1, uint32_t baseArrayLayer = 0)
    { return create_image_view(dbgName, image, viewType, {extent.width, extent.height, 1}, baseMipLevel, mipLevels, baseArrayLayer); }
    // global imageview
    spock::Image          create_image_and_view(const char* dbgName, VkExtent3D extent, VkFormat format, VkImageUsageFlags usage, VkImageViewType viewType, uint32_t mipLevels = 1);
    inline Image          create_image_and_view(const char* dbgName, VkExtent2D extent, VkFormat format, VkImageUsageFlags usage, VkImageViewType viewType, uint32_t mipLevels = 1)
    { return create_image_and_view(dbgName, {extent.width, extent.height, 1}, format, usage, viewType, mipLevels); }
    ...
    VkSemaphore create_semaphore(const char* dbgName);

    void                  destroy_image(Image image);

    Buffer                create_buffer(const char* dbgName, size_t allocSize, VkBufferUsageFlags usage, VkMemoryPropertyFlags requiredFlags, VmaAllocationCreateFlags flags = 0);
    Buffer                create_buffer(const char* dbgName, const void* data, size_t allocSize, VkBufferUsageFlags usage, VkMemoryPropertyFlags requiredFlags, VmaAllocationCreateFlags flags = 0);
