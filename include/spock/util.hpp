...
{
    spock::Buffer staging = spock::create_buffer("Staging", size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
inline void blit(VkCommandBuffer cmd, spock::Image& src, spock::Image& dst, VkRect2D srcRect, VkRect2D dstRect, VkFilter filter) {
...
inline void submit_command_buffer(VkQueue queue, std::initializer_list<VkCommandBuffer> commandBuffers, std::initializer_list<SemaphoreStagePair> waitSemaphores, std::initializer_list<SemaphoreStagePair> signalSemaphores, VkFence fence)
{
    ...
    static VkSemaphoreSubmitInfo waits[16];
    static VkSemaphoreSubmitInfo signals[16];

    uint32_t waitCount = 0;
    for (auto& w : waitSemaphores)
    {
        if (w.semaphore == VK_NULL_HANDLE) continue;
        waits[waitCount++] = {
    ...
    {
        if (s.semaphore == VK_NULL_HANDLE) continue;
        signals[signalCount++] = {
            .sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO,
